...
// COMMENTS TO GRADER:
// "Random light points" and "shuffle" are not used, but are computed within the code. 
// The image quality produced is better than without the computations.
// The implementation was influenced by books "Fundamentals of Computer Graphics" and "Realistic Ray Tracing" by Peter Shirley.
//============================================================
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    for(int i=0; i<SPP; ++i){
            light_points[i] = vec2(rand()*2.0-1.0, rand()*2.0-1.0)/2.0;
    }
    //Shuffle
    for(int i=0; i<SPP; ++i){
        int index = int(rand()*float(SPP));
        vec2 temp = light_points[i];
        light_points[i] = light_points[index];
        light_points[index] = temp;
    }
    ...
    for(int i=0; i<sqrt_SPP; ++i){
        for(int j=0; j<sqrt_SPP;++j){
            ...
            for ( int level = 0; level <= NUM_ITERATIONS; level++ ){
                ...
                vec3 r = normalize(reflect(nextRay.d, hitNormal));
                vec3 t = r;
                ...
                vec3 u = normalize(cross(t,r));
                vec3 v = cross(r,u);
                float a=0.4;
                vec2 rand = vec2(-a/2.0+rand()*a,-a/2.0+rand()*a);
                vec3 r_prime = r + u*rand.x+ v*rand.y;
                if(dot(hitNormal,r_prime)<0.0){
                    break;
                }
                nextRay = Ray_t( hitPos, normalize(r_prime) );
           }
