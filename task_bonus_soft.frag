...
// COMMENTS TO GRADER:
// The light size is a variable for each light. Both are modeled with the same size.
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    for(int i=0; i<sqrt_SPP; ++i){
        for(int j=0; j<sqrt_SPP;++j){
            Light[0].position.xz+=light_points[i*sqrt_SPP+j]*Light[0].size;
            Light[1].position.xz+=light_points[i*sqrt_SPP+j]*Light[1].size;
            //First, generate random number between [-0.5,0.5]
            vec2 rand_pos = vec2((rand()*2.0-1.0)/2.0, (rand()*2.0-1.0)/2.0);
            //Adjust to the aperture
            ...
            //Comment to turn off depth-of-field effect
            pixel_pos.xy+=pert;
